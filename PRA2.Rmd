...
Imputamos inicialnente con la media de la misma marca y tipo de cuerpo:
```{r echo=TRUE, message=FALSE, warning=FALSE}
data<-data %>%
...
Y si continua siendo `NA` imputamos por la media de la puntuación:
```{r echo=TRUE, message=FALSE, warning=FALSE}
data<-data %>%
...
```
Se puede observar que el boom de las cámaras digitales tuvo lugar entre el año 2000 y el año 2015. Posiblemente el declive se debió a mejora de las cámaras de los dispositivos móviles. Asimismo se puede observar, a través del código de colores, la evolución en el tipo de sensores que equipan las cámaras, pasando de sensores CCD al comienzo de la tecnología a sensores CMOS, que han copado el mercado desde el 2012.
